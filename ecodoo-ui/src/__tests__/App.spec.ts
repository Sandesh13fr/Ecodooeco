...
describe('App', () => {
  it('renders the Ecodoo landing route', async () => {
    ...
    const brandMarks = wrapper.findAll('img[src="/graphics/ecodoo-mark.webp"]')
    expect(brandMarks).toHaveLength(2)
    expect(brandMarks.every((mark) => mark.attributes('alt') === '')).toBe(true)
    expect(brandMarks.every((mark) => mark.attributes('width') === '32')).toBe(true)
    const icons = wrapper.findAll('svg[data-icon]')
    expect(icons).toHaveLength(9)
    expect(icons.every((icon) => icon.attributes('aria-hidden') === 'true')).toBe(true)
    expect(icons.every((icon) => icon.attributes('focusable') === 'false')).toBe(true)
  ...
  it('renders the MVP command center and summary routes', async () => {
    ...
    const brandMark = wrapper.get('img[src="/graphics/ecodoo-mark.webp"]')
    expect(brandMark.attributes('alt')).toBe('')
    expect(brandMark.attributes('width')).toBe('32')
    expect(brandMark.attributes('height')).toBe('32')
